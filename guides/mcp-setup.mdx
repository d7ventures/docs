...
<AccordionGroup>
  ...
  <Accordion title="Communication (Slack, Teams)" icon="message">
    - `list_channels` - Get available channels - `send_message` - Post message
    to channel - `get_users` - List team members - `upload_file` - Share files
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Method Not Found" icon="search">
    **Symptoms:** API returns "method not supported" error **Solutions:** -
    Check integration documentation for available methods - Verify integration
    supports the operation you're trying - Update to latest integration version
    - Contact support for custom method requests
  </Accordion>
