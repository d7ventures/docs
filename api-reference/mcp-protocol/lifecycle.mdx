...
Server capabilities supported by Kambrium:
...
Kambrium implements all standard server capabilities with proper negotiation.
...
Kambrium's HTTP transport supports graceful shutdown via session termination and connection closure.

## Timeouts

Implementations **SHOULD** establish timeouts for all sent requests, to prevent hung connections and resource exhaustion. When the request has not received a success or error response within the timeout period, the sender **SHOULD** issue a cancellation notification for that request and stop waiting for a response.

Kambrium implements proper timeout handling with cancellation support.
...
Kambrium maintains session state through the MCP lifecycle:
...
4. Session is now active for operations

### Session Validation

- Request ID uniqueness enforced per session
- Session timeout after periods of inactivity
- Authentication state maintained per session
...
When using HTTP transport, authentication is handled through:

### OAuth 2.1 Flow

1. Client obtains OAuth token through standard flow
2. Token included in `Authorization` header for all requests
3. Session tied to authenticated user for tool/resource access

### Personal Access Token (PAT)

1. User generates PAT through management interface
2. PAT included in `Authorization` header
3. Session validated against PAT permissions

Example authenticated initialization:
...
For detailed examples of the operation phase, see the MCP Methods documentation.
